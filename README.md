...
| Distribution | Model | KS statistic | Sim / Real mean | Measured on |
|---|---|---|---|---|
| Sessions per day | Negative Binomial (r=7.63, p=0.23) | 0.059 | 25.23 / 25.11 | twin output |
| Energy per session | Empirical resampling per connector | 0.009 | 30.43 / 30.44 | sampler |
| Session duration | Lognormal per connector | 0.037 | 0.76 / 0.74 | sampler |
| Arrival hour | Empirical hourly rates | 0.007 | 13.04 / 13.12 | twin output |

Energy uses empirical resampling, so its agreement is expected by construction. Only sessions per day and arrival hour are measured from simulator output; the other two test the fitted distributions rather than the simulation that consumes them.

**Arrival hour is the gate that matters most.** A KS test on daily session *counts* cannot see a corrupted arrival *profile* - the right number of sessions can be placed in the wrong hours, and waiting time is an emergent consequence of arrivals coinciding in time. An earlier defect did exactly that and went undetected until this check was added.
